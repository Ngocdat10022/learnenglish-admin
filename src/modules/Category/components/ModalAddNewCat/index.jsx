...
import { addNewCat } from "../../services";
import { useCatContext } from "@/contexts/CatContext";

export default function ModalAddNewCat({ handleSetShow }) {
  ...
  const handleAddNewCat = async (values) => {
    try {
      setLoading(true);
      const { data } = await addNewCat(values);
      toast.success(`${data}`);
      setLoading(false);
      reset({
        catName: "",
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center">
      <div className="ml-[256px] top-[100px] flex items-center justify-between p-[20px] rounded-md flex-col w-[800px] h-[800px] bg-blue-100">
        ...
        <form
          onSubmit={handleSubmit(handleAddNewCat)}
