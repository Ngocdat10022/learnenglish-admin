...
import { addNewLessons } from "../../service";

export default function ModalAddNewLesons({ handleSetShow }) {
  ...
  const handleAddNewLesson = async (values) => {
    console.log("value", values);
    try {
      setLoading(true);
      const { data } = await addNewLessons(values);
      toast.success(`${data}`);
      setLoading(false);
      reset({
        title: "",
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center">
      <div className="ml-[256px] top-[100px] flex items-center p-[20px] rounded-md flex-col w-[800px] h-[800px] bg-blue-100">
        ...
        <form
          onSubmit={handleSubmit(handleAddNewLesson)}
        ...
        >
          ...
          >
            {category?.map((item) => (
              <MenuItemCustom key={item?.catIdPK} value={item?.catIdPK}>
